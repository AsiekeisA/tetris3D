...
public class Playfield : MonoBehaviour
{
    ...
    {
        return ((int)pos.x >= 0 && (int)pos.x < gridSizeX && (int)pos.z >= 0 && (int)pos.z < gridSizeZ && (int)pos.y >=1);
    }
    ...
    public void SpawnNewBlock()
    {
        ...
        GameObject newBlock = Instantiate(blockList[randomBlock],spawnPoint,Quaternion.identity)as GameObject;

        //GameObject newGhost = Instantiate(ghostList[randomBlock], spawnPoint, Quaternion.identity) as GameObject;

        //newGhost.GetComponent<GhostBlock>().SetParent(newBlock);

        CalcPrv();
        Previewer.instance.ShowPreview(randomBlock);
    }

    public void CalcPrv()
    {
        randomBlock = Random.Range(0, blockList.Length);
    }
